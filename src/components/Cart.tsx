...
const Cart: React.FC<CartProps> = ({ isOpen, onClose, items, onPlaceOrder }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end">
      <div className="w-full max-w-md bg-black h-full overflow-y-auto border-l border-yellow-500/30">
        <div className="p-4 border-b border-yellow-500/30">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold bg-gradient-to-r from-yellow-400 to-yellow-600 bg-clip-text text-transparent">Your Cart</h2>
            <button
              onClick={onClose}
                ...
                <div key={item.id} className="flex items-center space-x-4 py-4 border-b border-gray-700">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className="bg-gray-800 text-white p-1 rounded hover:bg-gray-700 transition-colors border border-yellow-500/30"
                    >
                      <Minus size={16} />
                    </button>
